...
function DataTable(props) {
  ...
  function handlNewView(escena) {
    localStorage.setItem("escena",JSON.stringify(escena));
    ...
    <Paper sx={{ width: "100%", overflow: "hidden" }}>
      <TableContainer>
        <Table>
          <TableHead>
            <TableRow>
            <TableCell sx={{ fontWeight: "bold" }}>Nombre escena</TableCell>
            ...
            {data
              ...
              .map((item, index) => (
                <TableRow key={item.id}>
                  <TableCell> {item.nombreEscena}</TableCell>
